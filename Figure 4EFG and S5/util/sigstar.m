function varargout=sigstar(groups,stats,nosort)
    ...
    yd=myRange(y)*0.01; %separate sig bars vertically by 5% 
    for ii=1:length(groups)
...
%         y=repmat(y,4,1)+0.6;
        y = repmat(y,4,1)+0.2;
%         H(1)=plot(x(:),y+0.1,'-k','LineWidth',1.5,'Tag','sigstar_bar');
        ...
        x=repmat(x,2,1);
        y=repmat(y,4,1)+100;
        H(1)=plot(0*x(:),0*y,'-k','LineWidth',1.5,'Tag','sigstar_bar');
        starY=mean(y)+myRange(ylim)*0;
        H(2)=text(-1000*mean(x(:)),-100*starY,stars,...
            'HorizontalAlignment','Center',...
            'BackGroundColor','none',...
            'Tag','sigstar_stars','FontSize',20);
    end
